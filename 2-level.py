    ...
    def get_cluster_representations(self):
      ...
      for i in range(self.c):
        if self.which_type == 'min':
          ...
          cluster_values[i] = np.sum(self.rewards[i,:])/np.int(np.shape(self.rewards)[1])
...
TS = twolevel_TS(data.shape[1], data.shape[0],T)
KLUCB_min = twolevel_KL_UCB(data.shape[1], data.shape[0],'min', T)
KLUCB_max = twolevel_KL_UCB(data.shape[1], data.shape[0],'max', T)
KLUCB_avg = twolevel_KL_UCB(data.shape[1], data.shape[0],'avg', T)
